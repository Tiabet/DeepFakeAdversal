...
class DSFDDetector:
    ...
    def __init__(
            self,
            weight_path="dsfd_modified/weights/WIDERFace_DSFD_RES152.pth",
            nms_iou_threshold=.3,
            ):
    ...
    def detect_face(self, image_tensor, confidence_threshold, nms_iou_threshold):
        """
        Detect faces in an image tensor with gradients enabled.
        """
        # Ensure the input image requires gradients
        image_tensor.requires_grad = True

        # Preprocess the image (subtract mean values)
        image_tensor = image_tensor.float()
        image_tensor = image_tensor -  torch.tensor([104.0, 117.0, 123.0], device=image_tensor.device).view(1, 3, 1, 1)

        # Forward pass through the DSFD model
        output = self.net(image_tensor, confidence_threshold, nms_iou_threshold)

        # The output is already a tensor with gradients
        detections = output

        # Scale detections back to the original image size
        height, width = image_tensor.shape[2:]
        scale = torch.tensor([width, height, width, height], device=image_tensor.device)
        detections[:, :, 1:] *= scale

        # Return the detections with gradients preserved
        return detections[0]
